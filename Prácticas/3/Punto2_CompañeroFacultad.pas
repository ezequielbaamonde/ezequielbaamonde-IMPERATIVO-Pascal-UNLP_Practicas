...
//INICIO TIPOS DE DATOS | INCISO A --------------------------------------------------
	...
	datos_lis = record
		cod: integer;
		lis: listaVentas; //Apunta a un tipo de lista que contiene ventas
	end;
	arbol3 = ^nodo3;
	nodo3 = record {contiene solo codProducto y listaDeVentas}
		dato: datos_lis; //Contiene un registro con un dato de listas.
		HD: arbol3;
		HI: arbol3;
	end;
	...
	procedure agregarArbol3 (var a3: arbol3; dato: ventas);
	 ...
	 begin
		if (a3 = NIL) then begin
			...
			a3^.dato.lis:= nuevaVenta; //añado la lista generada al dato del arbol
			a3^.HI:= NIL;
			a3^.HD:= NIL;
		end
            ...
            nuevaVenta^.sig := a3^.dato.lis;
            a3^.dato.lis := nuevaVenta; //enganche
		end;
	 end;
